...
const { slashExecute } = require('./namehistory');
...
module.exports = {
	...
	async execute(client, message, args, Discord, prefix) {
		...
		try {
			...
			if (error.message === errors.PLAYER_DOES_NOT_EXIST) {
				...
				return message.reply({embeds: [neverLogged] }).then(() => {
					setTimeout(function() {
						message.delete()
					}, 5000);
				});
				...
				return message.reply({embeds: [error] }).then(() => {
					setTimeout(function() {
						message.delete()
					}, 5000);
				});
	...
	async slashExecute(client, Discord, interaction, serverDoc) {
		...
		try {
			...
			interaction.editReply({ embeds: [skin] });
			...
			if (error.message === errors.PLAYER_DOES_NOT_EXIST) {
				...
				const neverLogged = new Discord.MessageEmbed()
					...
					.setColor(colors["ErrorColor"])
					.setDescription("That player has never logged into Hypixel.");
				return interaction.editReply({embeds: [neverLogged] }).then(() => {
					setTimeout(function() {
						interaction.deleteReply()
					}, 5000);
				});
				...
				const error = new Discord.MessageEmbed()
					...
					.setColor(colors["ErrorColor"])
					.setDescription(`A problem has been detected and the command has been aborted, if this is the first time seeing this, check the error message for more details, if this error appears multiple times, DM \`ultiamte_hecker#1165\` with this error message \n \n \`Error:\` \n \`\`\`${error}\`\`\``)
				return interaction.editReply({embeds: [error] }).then(() => {
					setTimeout(function() {
						interaction.deleteReply()
					}, 5000);
				});
