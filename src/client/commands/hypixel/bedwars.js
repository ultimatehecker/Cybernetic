    ...
    async execute(client, message, args, Discord, prefix) {
        ...
        hypixel.getPlayer(player).then((player) => {
            const bedwars = new Discord.MessageEmbed()
                ...
                .addField("Averages per Game", `\`•\` **Kills**: \`${commaNumber(Math.ceil(player.stats.bedwars.kills / player.stats.bedwars.playedGames))}\` \n \`•\` **Final Kills**: \`${commaNumber(Math.ceil(player.stats.bedwars.finalKills / player.stats.bedwars.playedGames))}\` \n \`•\` **Beds Broken**: \`${commaNumber(Math.ceil(player.stats.bedwars.beds.broken / player.stats.bedwars.playedGames))}\``, true)
